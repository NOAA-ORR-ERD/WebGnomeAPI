...
If multiple winds are added to a model, only one will be used for weathering. This will be the first one listed in the wind panel. Making the first wind inactive will not cause the model to use the next wind for weathering. To use a different wind the original wind must be deleted. The name of the wind data being used for the weathering algorithms is displayed in the panel for reference.
